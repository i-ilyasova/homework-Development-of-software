...
from models import Faculty, Grade, Student, Subject, User
        ...
        for row in csv.DictReader(f):
            ...
            if student_key not in students:
                existing = db.query(Student).filter(
                    Student.last_name == last_name, Student.first_name == first_name
                ).first()
...
def create_user(db: Session, username: str, email: str, hashed_password: str, role: str = "admin") -> User:
    obj = User(username=username, email=email, hashed_password=hashed_password, role=role)
    db.add(obj)
    db.commit()
    db.refresh(obj)
    return obj
